...
(function () {
  ...
  /** Pinta de rojo el acuerdo que falta y lo trae a la vista. */
  function marcarFalta(id, falta) {
    var caja = $(id).closest('.acuerdo');
    if (!caja) return;
    caja.classList.remove('falta');
    if (falta) {
      void caja.offsetWidth;               // reinicia la animación si ya estaba puesta
  ...
  function guardar() {
    ...
    var datos = {
      ...
      aceptaDatos: $('ckDatos').checked,
      aceptaPremio: $('ckPremio').checked
    };

    /* Los dos acuerdos van primero: sin ellos no se envía nada al servidor. */
    marcarFalta('ckDatos', false); marcarFalta('ckPremio', false);
    var err = '';
    if (!datos.aceptaDatos) {
      err = 'Para continuar debes aceptar el tratamiento de tus datos personales.';
      marcarFalta('ckDatos', true);
    } else if (!datos.aceptaPremio) {
      err = 'Para continuar debes aceptar presentar el documento físico del adulto responsable en la premiación.';
      marcarFalta('ckPremio', true);
    }
    else if (datos.nombres.length < 2) err = 'Escribe los nombres del adulto responsable.';
  ...
  function volverInicio() {
    ...
    estado.categoria = '';
    $('ckDatos').checked = false; $('ckPremio').checked = false;
    marcarFalta('ckDatos', false); marcarFalta('ckPremio', false);
  ...
  function arrancar() {
    ...
    ['ckDatos', 'ckPremio'].forEach(function (id) {
      $(id).addEventListener('change', function () { if (this.checked) marcarFalta(id, false); });
    });
